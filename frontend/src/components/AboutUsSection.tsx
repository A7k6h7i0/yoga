...
import { 
  ...
  Target, Globe, Sparkles, ShieldCheck, TrendingUp,
  Heart
} from 'lucide-react';
...
const AboutUsSection = () => {
  ...
  const features = [
    ...
    {
      title: 'For Everyone',
      desc: 'Individuals, Kids, Seniors & Families',
      icon: Globe
    },
    {
    ...
    <section className="py-24 bg-[#FDFCFB] text-sky-950 overflow-hidden">
      <div className="w-full px-4 md:px-8 lg:px-20">
        <div className="flex flex-col lg:flex-row gap-16 lg:gap-24 mb-24 items-center">
          ...
          {/* Right Column: Visual Mosaic (Two Flex Columns to remove vertical space) */}
          <div className="lg:w-[55%] flex gap-0 relative z-10 self-center">
            {/* Column 1: Corporate (Short) & Live Classes (Big) */}
            <div className="flex-1 flex flex-col gap-0">
              {/* Card 1: Corporate Wellness */}
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                className="bg-white rounded-tl-[2.5rem] shadow-xl flex flex-col group cursor-pointer border border-slate-50 overflow-hidden relative h-[240px] md:h-[300px]"
              >
              ...
              <motion.div
                ...
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                className="bg-white rounded-bl-[2.5rem] shadow-xl flex flex-col group cursor-pointer border border-slate-50 overflow-hidden relative h-[320px] md:h-[400px]"
              >
            ...
            {/* Column 2: Individual (Big) & Programs (Short) */}
            <div className="flex-1 flex flex-col gap-0">
              {/* Card 2: Individual Wellness */}
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                className="bg-white rounded-tr-[2.5rem] shadow-xl flex flex-col group cursor-pointer border border-slate-50 overflow-hidden relative h-[320px] md:h-[400px]"
              >
              ...
              {/* Card 4: Programs & Challenges */}
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                className="bg-white rounded-br-[2.5rem] shadow-xl flex flex-col group cursor-pointer border border-slate-50 overflow-hidden relative h-[240px] md:h-[300px]"
              >
